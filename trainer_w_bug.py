...
def load_data(folder):
    ...
    for file in files:
        ...
        for t in range(num_rows):
            ...
            for_pre_process =  current_data.iloc[:, :-1]
            pct_miss = for_pre_process.isna().sum()/for_pre_process.shape[0]
            current_data.ffill()
            six_mean = six_hr_window(for_pre_process).mean()
            six_std = six_hr_window(for_pre_process).std()
            out = pd.concat([pct_miss, six_mean, six_std, current_data.iloc[-1]])
