...
load_challenge_data = function(file){
    ...
    nc <- ncol(data)
    if (column_names[nc] == 'SepsisLabel'){
        data <- data[,-c(nc)]
    }
...
for (f in list.files(input_directory)){
    if (file.exists(file.path(input_directory, f)) && nchar(f) >= 3 && substr(f, 1, 1) != '.' && substr(f, nchar(f)-2, nchar(f)) == 'csv'){
        files = c(files, f)
    }
}
...
for (file in files){
    ...
    num_rows <- nrow(data)
    num_cols <- ncol(data)
    predictions <- matrix(NA, num_rows, 2)
    for (t in 1:num_rows){
